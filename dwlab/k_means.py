...
X_train = np.array([[2, 10], [2, 5], [8, 4], [5, 8], [7, 5], [6, 4]])

# Fit centroids to dataset
kmeans = KMeans(n_clusters=2)  # Set number of clusters to 2
kmeans.fit(X_train)
...
sns.scatterplot(
    x=[X[0] for X in X_train],
    y=[X[1] for X in X_train],
    hue=classification,  # Use classification instead of true_labels
    palette="deep",
    legend=None,
)
plt.plot(
...
# Iterate through the classification results and organize points into clusters
for i, point in enumerate(X_train):
    if classification[i] == 0:
        cluster1.append(tuple(point))
    elif classification[i] == 1:
        cluster2.append(tuple(point))
